<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="font-display text-2xl font-semibold text-teal-950">{{ isset($halaqah) ? 'Ubah halaqah' : 'Buat halaqah' }}</h1>
        </div>
    </x-slot>
    ...
    <form method="POST" action="{{ $action }}" class="max-w-xl ui-card p-5 sm:p-6 grid gap-4">
        ...
        <div>
            ...
            <x-text-input id="name" name="name" class="mt-1.5" :value="old('name', $halaqah?->name)" required placeholder="Halaqah Tahfidz A" />
            <x-input-error class="mt-2" :messages="$errors->get('name')" />
        </div>
        <div>
            <x-input-label for="academic_year_id" value="Tahun ajaran" />
            <select id="academic_year_id" name="academic_year_id" class="mt-1.5 ui-input" required>
        ...
        <div>
            ...
            <select id="ustaz_user_id" name="ustaz_user_id" class="mt-1.5 ui-input" required>
