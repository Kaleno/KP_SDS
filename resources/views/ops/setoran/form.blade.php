...
<div class="space-y-4">
    ...
    @else
        <div>
            ...
            <select id="santri_id" name="santri_id" required
                    class="mt-1 w-full rounded-xl border-slate-300 text-base min-h-12">
    ...
    <div>
        ...
        <x-text-input id="setoran_date" type="date" name="setoran_date" class="mt-1 block w-full min-h-12 text-base rounded-xl"
    ...
    <div>
        ...
        <select id="quran_surah_id" name="quran_surah_id" required
                class="mt-1 w-full rounded-xl border-slate-300 text-base min-h-12">
    ...
    <div class="grid grid-cols-2 gap-3">
        <div>
            ...
            <x-text-input id="ayah_start" type="number" min="1" name="ayah_start" class="mt-1 block w-full min-h-12 text-base rounded-xl"
        ...
        <div>
            ...
            <x-text-input id="ayah_end" type="number" min="1" name="ayah_end" class="mt-1 block w-full min-h-12 text-base rounded-xl"
    ...
    <div>
        <p class="text-sm font-medium text-slate-700 mb-2">Status</p>
        <div class="grid grid-cols-3 gap-2">
            @foreach ($statuses as $status)
                <label class="block">
                    ...
                    <span class="flex items-center justify-center min-h-12 rounded-xl border border-slate-300 text-sm font-semibold text-slate-700 {{ $status->buttonClass() }}">
                        {{ $status->label() }}
                    </span>
                </label>
            @endforeach
    ...
    <div>
        ...
        <textarea id="note" name="note" rows="2"
                  class="mt-1 w-full rounded-xl border-slate-300 text-base">{{ old('note', $setoran?->note) }}</textarea>
    ...
    @if ($isEdit)
        <div>
            ...
            <textarea id="correction_note" name="correction_note" rows="2" required
                      class="mt-1 w-full rounded-xl border-slate-300 text-base">{{ old('correction_note', $setoran?->correction_note) }}</textarea>
